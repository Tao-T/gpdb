...
static void plan_jit_flags(PlannedStmt* pstmt);
...
/*
 * Decide whether to active JIT and record the actived JIT subfeatures in
...
static void plan_jit_flags(PlannedStmt* pstmt)
{
	...
	if (NULL == pstmt)
	{
		/* ORCA optimizer can fail and return a null pointer */
		return;
	}
