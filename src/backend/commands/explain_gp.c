		...
		if (es->analyze && es->timing)
		{
			...
			if (es->format == EXPLAIN_FORMAT_TEXT)
			{
				appendStringInfo(es->str, "//%.3f ms avg x %d workers, %.3f ms max (seg%d)",
								 1000.0 * avg_time, ss->nworker, 1000.0 * max_time, ss->segindex0 + idx);
				appendStringInfo(es->str, "//end\n");
			}
			else
			{
				ExplainPropertyFloat("Avg", "ms", 1000.0 * avg_time, 3, es);
				ExplainPropertyFloat("Max", "ms", 1000.0 * max_time, 3, es);
				ExplainCloseGroup("Timing", "Timing", true, es);
			}
		}
		es->indent -= 1;
